/* kmeans_1d_openmp.c
   K-means 1D (C99), implementação paralela com OpenMP:
   ...
   Compilar: gcc -O2 -std=c99 openmp.c -o openmp -lm -fopenmp
   Uso:      ./openmp dados.csv centroides_iniciais.csv [max_iter=50] [eps=1e-4] [assign.csv] [centroids.csv]
*/
...
static void update_step_1d(const double *X, double *C, const int *assign, int N, int K){
    ...
    // Create thread-local sum and cnt arrays
    int num_threads;
    #pragma omp parallel
    {
        #pragma omp single
    ...
    double **sum_thread = (double**)malloc(num_threads * sizeof(double*));
    int **cnt_thread = (int**)malloc(num_threads * sizeof(int*));
    ...
    for(int t=0; t<num_threads; t++){
        ...
        if(!sum_thread[t] || !cnt_thread[t]){ 
            fprintf(stderr,"Sem memoria no update (threads)\n"); 
            exit(1); 
        }
    }
...
int main(int argc, char **argv){
    ...
    // Measure time using both methods
    ...
    clock_t t1_clock = clock();
    double t1_omp = omp_get_wtime();
    
    double ms_omp = 1000.0 * (t1_omp - t0_omp);
    double ms_clock = 1000.0 * (t1_clock - t0_clock) / CLOCKS_PER_SEC;

    printf("K-means 1D (OpenMP)\n");
    printf("N=%d K=%d max_iter=%d eps=%g\n", N, K, max_iter, eps);
    printf("Número de threads: %d\n", omp_get_max_threads());
    printf("Iterações: %d | SSE final: %.6f\n", iters, sse);
    printf("Tempo (omp_get_wtime): %.6f ms\n", ms_omp);
    printf("Tempo (clock): %.6f ms\n", ms_clock);
